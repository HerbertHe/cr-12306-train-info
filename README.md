...
- **并发控制**：通过任务调度器 `TaskScheduler` 自适应调整并发（范围 2 ~ 8），成功率高自动提速，失败率高自动降速，任务启动随机延迟降低被识别概率
...
- **并发与任务调度**：
  ...
  - 默认并发范围为 2 ~ 8，自适应调整：成功率 > 95% 增加并发，< 80% 减少并发
  - 每个任务启动随机延迟 200 ~ 800ms，降低被识别为爬虫概率
  - 如需调整范围可修改 `src/index.ts` 中的 `new TaskScheduler(8, 2)`
- **代理与网络**：
